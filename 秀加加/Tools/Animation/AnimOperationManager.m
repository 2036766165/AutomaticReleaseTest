...
- (void)animWithUserID:(NSString *)userID model:(GiftModel *)model finishedBlock:(void(^)(BOOL result))finishedBlock {
    ...
    if ([self.userGigtInfos objectForKey:userID]) {
        ...
        AnimOperation *op = [AnimOperation animOperationWithUserID:userID model:model finishedBlock:^(BOOL result,NSInteger finishCount) {
            ...
            // 延时删除用户礼物信息
            NSInteger totalCount = self.queue1.operationCount + self.queue2.operationCount;
        ...
        if ([userID integerValue] % 2) {
            ...
            if (op.model.giftCount != 0) {
                ...
                if (WKScreenW > WKScreenH) {
                    op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, WKScreenH/2 - 120, 180, 40);
                }else{
                    op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, WKScreenH/2 - 60, 180, 40);
                }
            ...
            if (op.model.giftCount != 0) {
                if (WKScreenW > WKScreenH) {
                    ...
                    op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, WKScreenH/2, 180, 40);
                }
    ...
    {   // 如果有操作缓存，则直接累加，不需要重新创建op
        ...
        if ([userID integerValue] % 2) {
            ...
            if (op.model.giftCount != 0) {
                ...
                if (WKScreenW > WKScreenH) {
                    op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, WKScreenH/2 - 80, 180, 40);
                }else{
                    op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, WKScreenH/2 - 60, 180, 40);
                }
